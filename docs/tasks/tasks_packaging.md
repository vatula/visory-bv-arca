...
* [x] **Feature — Queue endpoint (`GET /api/v1/queue`):** Added `resources_path` field to `Settings` (bound to `RESOURCES_PATH` env var from `docker-compose.yaml`). Created `backend/src/api/routes/queue.py` with `_RawBankAccount`/`_RawTransaction` TypedDicts for strict mypy compliance. Returns next 10 unprocessed transactions (filtered against `arcra_ui_read_model`) in feed order plus `total_remaining` count.
* [x] **Feature — Frontend queue panel and Play/Pause control:** Added `QueuedTransactionSchema`/`QueueResponseSchema` Zod schemas and extended `DashboardState` with `queue`/`totalRemaining`. Added `fetchQueue()` and `submitTransaction()` to `api.ts`. Created `dashboard/page.tsx` as a client component with `isPlaying` state, sequential processing loop guarded by `processingRef`, and a Play/Pause button. Created `QueuePanel.tsx` and updated `DashboardLayout.tsx` to render the queue panel above the two-column grid.
* [x] **Runtime Fix — Proxy route handler missing from disk:** `src/app/api/[...path]/route.ts` was not written to disk in the previous session (shell `mkdir -p` silently dropped the bracket directory name). Recreated using the dedicated file creation tool; confirmed present in standalone build and all three proxied endpoints return correct JSON.