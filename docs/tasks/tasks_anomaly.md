...
- [ ] **Service Layer:** Initialize the `pydantic_ai.models.bedrock.BedrockModel`. Enforce strict JSON output schemas via Pydantic `result_type` for anomaly routing.
- [ ] **Node Implementation (`ExtractLedgerVarianceNode`):** Initialize the graph execution thread. Parse the Xero context to prepare for Pydantic AI `Agent` evaluation.
- [ ] **Node Implementation (`EvaluateVaguenessNode`):** Pass transaction details to the Bedrock-powered `Agent`. 
